    ...
    def _prepare_inputs(self, inputs: dict[str, Union[torch.Tensor, Any]]) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        if self.args.use_vllm:
            ...
            if self.accelerator.is_main_process:
                ...
                completion_logprobs = [None] * len(all_prompts_text) * self.num_generations

            # Broadcast both completions and logprobs
            completion_ids, completion_logprobs = broadcast_object_list(
                [completion_ids, completion_logprobs], from_process=0
            )
            process_slice = slice(
            ...
            completion_logprobs = completion_logprobs[process_slice]
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, nn.Module):  # Module instead of PretrainedModel for compat with compiled models
                ...
                # Add completion_logprobs to the reward function arguments
                if self.args.use_vllm:
                    reward_kwargs["completion_logprobs"] = completion_logprobs
                else:
                    # Convert scores to log probabilities for the regular generation path
                    completion_logprobs = [torch.log_softmax(score, dim=-1) for score in scores]
                    reward_kwargs["completion_logprobs"] = completion_logprobs
